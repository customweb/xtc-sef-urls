...
if ($_GET['action'])
{
	...
	{
		case 'save':
			$configuration_query = xtc_db_query("select configuration_key,configuration_id, configuration_value, use_function,set_function from " . TABLE_CONFIGURATION . " where configuration_group_id = '" . CONFIGURATION_GROUP_ID . "' order by sort_order");
			...
			xtc_redirect(FILENAME_SEF_URLS. '?file=config');
			break;
...
echo xtc_draw_form('configuration', FILENAME_SEF_URLS, 'file=' . $_GET['file'] . '&action=save');
